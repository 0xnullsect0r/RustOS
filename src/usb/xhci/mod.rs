...
use ring::{CommandRing, EventRing, TransferRing, RING_SIZE};
use trb::*;
...
impl EndpointDesc {
    pub fn is_bulk_in(&self)  -> bool { (self.attributes & 0x3) == 2 && (self.addr & 0x80) != 0 }
    pub fn is_bulk_out(&self) -> bool { (self.attributes & 0x3) == 2 && (self.addr & 0x80) == 0 }
    pub fn ep_num(&self) -> u8 { self.addr & 0x7F }
}
...
unsafe fn write64(base: u64, off: usize, val: u64) {
    unsafe { core::ptr::write_volatile((base + off as u64) as *mut u64, val); }
}
...
impl Xhci {
    ...
    pub fn init(pci_dev: &PciDevice) -> Option<Self> {
        ...
        crate::serial_println!("[xhci] MMIO base virt=0x{:x} phys=0x{:x}", mmio_base, bar_phys);
        ...
        unsafe {
            ...
            spin_until(|| read32(op_base, OP_USBCMD) & CMD_HCRST == 0, "HCRST clear");
        ...
        let mut ctrl = Xhci {
            mmio_base, op_base, rt_base, db_base,
            max_ports, max_slots,
            cmd_ring, event_ring,
    ...
    fn ring_doorbell(&self, slot: u8, endpoint: u8) {
        ...
        unsafe { write32(self.db_base, db_off as usize, endpoint as u32); }
    }
    ...
    fn wait_event(&mut self, expected_type: u32) -> Option<Trb> {
        for _ in 0..10_000_000u64 {
            if let Some(evt) = self.event_ring.pop() {
                ...
                unsafe { write64(self.rt_base, RT_IR0_ERDP, dp | (1 << 3)); }
    ...
    fn reset_port(&mut self, port: u8) {
        ...
        unsafe { core::ptr::write_volatile(port_reg as *mut u32, (portsc | PORTSC_PR) & !(PORTSC_PRC | PORTSC_CSC)); }
        ...
        // Clear PRC
        let portsc = unsafe { core::ptr::read_volatile(port_reg as *const u32) };
        unsafe { core::ptr::write_volatile(port_reg as *mut u32, portsc | PORTSC_PRC); }
    ...
    fn enable_slot(&mut self, port: u8, speed: u8) {
        ...
        unsafe { unsafe { core::ptr::write_bytes(dev_ctx_virt, 0, core::mem::size_of::<DevCtx>()); } }

        // Store in DCBAA
        unsafe { self.dcbaa.add(slot_id as usize).write_volatile(dev_ctx_phys); }

        // Allocate Input Context
        let (in_ctx_virt, in_ctx_phys) = dma_alloc(core::mem::size_of::<InputCtx>(), 64);
        unsafe { core::ptr::write_bytes(in_ctx_virt, 0, core::mem::size_of::<InputCtx>()); }
        ...
        if self.control_in(&mut dev, 0x80, 6, 0x0100, 0, 18, buf_phys).is_some() {
        ...
        if self.control_in(&mut dev, 0x80, 6, 0x0200, 0, cfg_len as u16, cfg_phys).is_none() {
            ...
            match dtype {
                ...
                0x05 => { // Endpoint
                    if is_msc && len >= 7 {
                        ...
                        if ep.is_bulk_in()  { bulk_in  = Some(ep.clone()); }
                        if ep.is_bulk_out() { bulk_out = Some(ep); }
                    }
                }
                _ => {}
            }
        ...
        crate::serial_println!("[xhci] MSC EP IN=0x{:x} OUT=0x{:x}", bulk_in.addr, bulk_out.addr);
        ...
        unsafe {
            ...
            in_ctx2.ctrl.add_flags = (1 << ep_dci(bulk_in.ep_num(), true))
                                   | (1 << ep_dci(bulk_out.ep_num(), false));
        ...
        if evt.completion_code() != cc::SUCCESS {
            crate::serial_println!("[xhci] Configure Endpoint failed cc={}", evt.completion_code());
            return;
        }
    ...
    fn control_in(
        &mut self, dev: &mut UsbDevice,
        bm: u8, breq: u8, wval: u16, widx: u16, wlen: u16,
        buf_phys: u64,
    ) -> Option<()> {
        dev.ctrl_ring.push(setup_stage_trb(bm, breq, wval, widx, wlen, 3, true));
        if wlen > 0 {
            dev.ctrl_ring.push(data_stage_trb(buf_phys, wlen as u32, true, true));
        }
    ...
    fn control_out(
        ...
        dev.ctrl_ring.push(setup_stage_trb(bm, breq, wval, widx, wlen, 0, true));
        dev.ctrl_ring.push(status_stage_trb(true, true));

        let slot = dev.slot_id;
        self.ring_doorbell(slot, 1);
        let evt = self.wait_event(ty::TRANSFER_EVENT)?;
        if evt.completion_code() != cc::SUCCESS { return None; }
        Some(())
    }
    ...
    pub fn bulk_out(&mut self, dev_idx: usize, data: &[u8]) -> bool {
        ...
        unsafe { core::ptr::copy_nonoverlapping(data.as_ptr(), buf_virt, data.len()); }
        let slot_id  = self.devices[dev_idx].slot_id;
        let ep_dci_v = ep_dci(self.devices[dev_idx].bulk_out_ep, false) as u8;
        self.devices[dev_idx].bulk_out_ring.push(normal_trb(buf_phys, data.len() as u32, true));
        self.ring_doorbell(slot_id, ep_dci_v);
        let evt = self.wait_event(ty::TRANSFER_EVENT);
        evt.map(|e| e.completion_code() == cc::SUCCESS).unwrap_or(false)
    }
    ...
    pub fn bulk_in(&mut self, dev_idx: usize, len: usize) -> Option<alloc::vec::Vec<u8>> {
        ...
        self.devices[dev_idx].bulk_in_ring.push(normal_trb(buf_phys, len as u32, true));
        self.ring_doorbell(slot_id, ep_dci_v);
        let evt = self.wait_event(ty::TRANSFER_EVENT)?;
        if evt.completion_code() != cc::SUCCESS && evt.completion_code() != cc::SHORT_PACKET {
            return None;
    ...
    fn scsi_command(&mut self, dev_idx: usize, cdb: &[u8], data_in: Option<usize>) -> Option<alloc::vec::Vec<u8>> {
...
/// Peek the current producer cycle bit of a ring without advancing it.
fn ring_cycle(ring: &TransferRing) -> bool {
    // Read cycle_bit field - we need to expose it; for now use a fixed true
    // (cycle tracking is handled internally by push())
    true
}
